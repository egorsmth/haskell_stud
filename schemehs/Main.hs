...
eval env (List [Atom "cond", (List (x:xs))]) = 
     do result <- cond env x
        case result of
             Bool False -> eval env (List [Atom "cond", (List xs)])
             otherwise  -> return result
eval env (List [Atom "cond", (List [])]) = return $ Bool False
...
binop ::(a -> Bool) -> [a] -> ThrowsError LispVal
binop op params = return $Bool $ foldl1 (&&) $ map op params
...
runRepl :: IO ()
runRepl = nullEnv >>= until_ (== "quit") (readPrompt "l>>") . evalAndPrint
